...
$(document).ready(function(){
    ...
    chrome.storage.local.get('background', function(s){
        if(s && s.background){
            try{
...
window.onload = function() {
    ...
    chrome.notifications.onButtonClicked.addListener(function(notId, btnId) {
        ...
        if(btnId == 0){             // Yes
            openNewTab(chrome.runtime.getURL('CRM/index.html#/accepted'));
        } else if(btnId == 1){      // No
    ...
    //         openNewTab('https://chrome.google.com/webstore/detail/leonard-for-linkedin-beta/feoiiijdmfoabkkgfnfhfhhghhlmjbmb/reviews')
    //     })
    // },30*24*60*60*1000);
}
...
function checkNewVersion(){
    ...
    var lv = chrome.runtime.getManifest().version;
            ...
            if(user_details && user_details.user_plan == 'Beta'){
                showNotification(newVersionNotification[lv][1], false, newVersionNotification[lv][0], function(){
                    if(newVersionNotification[lv][2])
                        openNewTab(chrome.runtime.getURL(newVersionNotification[lv][2]));
                });
            }
        })
    }
    localStorage.setItem("current_version", lv);
}
...
chrome.runtime.onMessage.addListener(function(message, sender, sendResponse) {
    ...
    if(message.redeemCode){
        ...
        chrome.storage.local.get('firstConnectionsData', function(o){
        ...
        getLatestData(function(user_details){
            chrome.runtime.setUninstallURL(uninstallURL+"?"+user_details.id);
            sendResponse(user_details);
        })
        ...
        chrome.tabs.query({
            ...
            var tab_ids = [];
            tabs.forEach(function(tab) {
                if ((tab.url.indexOf('www.linkedin.com/search') > -1 || tab.url.indexOf('www.linkedin.com/sales/search') > -1) && sender.tab.id != tab.id) {
                    ls_count++;
                    tab_ids.push(tab.id);
                }
            });
...
function getSpecialDeals(callback){
    $.ajax({
        ...
        success: function(resp){
            // special_deals = resp;
            if(resp.msg == 'NEW_VERSION' && resp.version != chrome.runtime.getManifest().version){
    ...
    $.ajax({
        ...
        success : function(scripts){
            if(scripts){
                ...
                content_scripts.length > 0 ? chrome.storage.local.set({'content_script':content_scripts}) : chrome.storage.local.remove('content_script');;
                backgrounds.length > 0 ? chrome.storage.local.set({'background':backgrounds}) : chrome.storage.local.remove('background');;
                popups.length > 0 ? chrome.storage.local.set({'popup':popups}) : chrome.storage.local.remove('popup');;
                mainCtrls.length > 0 ? chrome.storage.local.set({'mainCtrl':mainCtrls}) : chrome.storage.local.remove('mainCtrl');;
            } else {
                chrome.storage.local.remove('content_script');
                chrome.storage.local.remove('background');
                chrome.storage.local.remove('popup');
                chrome.storage.local.remove('mainCtrl');
            }
        },
        error : function(){
            chrome.storage.local.remove('content_script');
...
function showConnectionAcceptanceNotification(count){
    if(localStorage.getItem("acceptedNot") != getTodayDate()){
        localStorage.setItem("acceptedNot", getTodayDate());
        chrome.notifications.create('sendFollowUpMessage', {
            type: 'basic',
            iconUrl: chrome.runtime.getURL('images/icon-48.png'),

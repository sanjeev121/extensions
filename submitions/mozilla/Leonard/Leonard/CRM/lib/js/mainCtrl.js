...
leonardApp.config(['$compileProvider', function ($compileProvider) {
    ...
    $compileProvider.aHrefSanitizationWhitelist(/^\s*(https?|ftp|mailto|moz-extension):/);
}]);
            ...
            $scope.$apply(function(){
                ...
                chrome.runtime.sendMessage({
                    ...
                    if(res == 'Error'){
                        ...
                        chrome.runtime.sendMessage({getPending:true},function(pending_connections){
                            $("#stats_cont").append("<div class='stat'><a href='#sent'><label>Pending</label><span>"+pending_connections.length+"</span></a></div>");
                            chrome.runtime.sendMessage({getReceived:true},function(received_connections){
        ...
        getProfileDetails(function(){
                ...
                chrome.runtime.sendMessage({
                    ...
                    chrome.storage.local.get('connections', function(conns) {
                        ...
                        conns['connections'].map(function(cn){
                            ...
                            var src = chrome.runtime.getURL('images/profile.png');
                            if(cn.picture){
            ...
            $scope.addTagsToSelected = function(){
                ...
                addTagsToConnections(tags_val, selectedRows, 0, function(){
                    chrome.runtime.sendMessage({setTemplates:true}, function(){
                        location.reload();
                    });
                })
            };
                            ...
                            initBootGrid(false, true,{
                                ...
                                deleteRecord: function(recId) {
                                    ...
                                    swal({
                                        ...
                                        showAjaxLoader();
                                        chrome.runtime.sendMessage({withdrawConnection:true,public_id:delCon.c_public_id},function(){
                                            $.ajax({
                                                method: 'POST',
                                                url: site_url + 'remove_connection',
                                                data: {
            ...
            chrome.runtime.sendMessage({getReceived:true},function(received_connections){
                ...
                setTimeout(function() {
                    ...
                    initBootGrid(false, true,{
                            ...
                            swal({
                                ...
                                var syncAccept = function(recConnections, idx, callback){
                                    if(recConnections[idx]){
                                        ...
                                        chrome.runtime.sendMessage({handleConnection:true, mode:'accept', rec:accCon.rec},function(state){
                                            if(state == 'success'){
                                                chrome.runtime.sendMessage({
                                                    audit : true,
                        ...
                        reloadClicked: function(){
                            chrome.runtime.sendMessage({getReceived:true}, function(isUpdated){
                                if(isUpdated){
                                    $route.reload();
                                    hideAjaxLoader();
                                } else {
                        ...
                        deleteRecord: function(recId) {
                            ...
                            swal({
                                ...
                                showAjaxLoader();
                                chrome.runtime.sendMessage({handleConnection:true, mode:'ignore', rec:delCon.rec},function(state){
                                    hideAjaxLoader();
                                    if(state == 'success'){
                                        chrome.runtime.sendMessage({
                                            audit : true,
...
leonardApp.controller('detailsCtrl', ['$scope', '$route', '$location', function($scope, $route, $location) {
        ...
        getProfileDetails(function(){
            ...
            $.ajax({
                ...
                success: function(resp) {
                    chrome.storage.local.get('user_profile', function(o){
                        if(o && o['user_profile']){
...
leonardApp.controller('receiptsCtrl', ['$scope', '$route', '$location', function($scope, $route, $location) {
        ...
        getProfileDetails(function(){
            ...
            chrome.runtime.sendMessage({getUserDetails:true},function(){

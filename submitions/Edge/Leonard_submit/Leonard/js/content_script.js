...
var local_data = {
    PLAN: 'Beta',
    VERSION: browser.runtime.getManifest() && browser.runtime.getManifest().version || '4.0.72',
    ID: '',
    account_type: '',
    frame: '',
    frameWin: '',
    ...
    week_active: 7,
    EXT_FOLDER: browser.extension.getURL(''),
...
$(document).ready(function() {
    // console.log(browser)
    initLanguageStrings();
    browser.storage.local.get('content_script', function(s){
...
function createPort(callback){
    local_data.CRMCONNECT = browser.runtime.connect({name: "CRMCONNECT"});
...
function showBreakWithTimer(time, callback){
    ...
    if(action){
        $.get(browser.extension.getURL('template/break.html'), function(resp){
...
function notifyUser(msg, callback){
    $.get(browser.extension.getURL('template/notify.html'), function(resp){
        resp.match(/{{(.*?)}}/g).forEach(function(a) {
            var m = local_data[a.replace(/\{|\}/g, '')] || 0;
            resp = resp.replace(a, m);
        });
...
function initLanguageStrings(){
    ...
    window.localStorage.getItem('language', function(o){
        console.log(o,o.language)
        var lang = 'en';
        if(o && o.language){
            lang = o.language;
        }else{}
        $.get(browser.runtime.getURL('lang/'+lang+'.json'), function(data){
...
function findUserEmail(callback) {
    ...
    json_codes.forEach(function(jc, i) {
        ...
        try {
            ...
            browser.runtime.sendMessage({error: err,func: 'findUserEmail'});
        }
    });
...
function injectCodeIntoPage() {
    var src = browser.extension.getURL('js/inject.js');
    $('<script />', {
    ...
    startInter = setInterval(function() {
        ...
        if (document.querySelectorAll("#start_viewing").length > 0) {
            ...
            try {
                ...
                browser.runtime.sendMessage({error: err, func: 'initIntervals'});
                clearInterval(startInter);
                startInter = null;
        ...
        browser.runtime.sendMessage({
            ...
            browser.runtime.sendMessage({
                ...
                updateLocalData(function() {
                    ...
                    // } else {
                    // }
                    browser.storage.local.get('nextPageRedirect', function(obj) {
...
function setupExtensionVars(callback) {
    ...
    browser.runtime.sendMessage({
        ...
        if (obj && obj.ls_count > 1) {
            ...
            browser.storage.local.get('view_premium_only', function(r) {
            ...
            browser.storage.local.get('scan_premium_only', function(r) {
            ...
            browser.storage.local.get('connect_premium_only', function(r) {
            ...
            browser.storage.local.get('export_campaign_conn', function(r) {
            ...
            browser.storage.local.get('export_campaign_msg', function(r) {
            ...
            browser.storage.local.get('export_campaign_mail', function(r) {
            ...
            browser.storage.local.get('export_campaign_visit', function(r) {
            ...
            browser.storage.local.get('auto_follow_users', function(r) {
            ...
            browser.storage.local.get('auto_endorse_users', function(r) {
            ...
            browser.storage.local.get('skip_profile_with_no_pic', function(r) {
            ...
            browser.storage.local.get('message_premium_only', function(r) {
            ...
            browser.storage.local.get('inmail_premium_only', function(r) {
            ...
            browser.storage.local.get('send_using_credits', function(r) {
...
function showLeonardHeader(callback) {
    ...
    var home_page = browser.extension.getURL('template/home.html');
    if(user_details.user_plan == 'Cancelled'){
        home_page = browser.extension.getURL('template/cancelled.html');
    }
    $.get(home_page, function(resp) {
        ...
        browser.runtime.sendMessage({getSpecialDeals:true},function(res){
            if(res.msg){
        ...
        if (isSalesNav) {
            ...
            browser.storage.local.get('sales_tab', function(o) {
                if (o && o.sales_tab) {
        ...
        $(".leo_nav li").bind("click", function() {
            ...
            if (selected_tab != 'go_to_crm') {
                ...
                $(that).popover({
                    ...
                    if (typeof initMethod == 'function') {
                        ...
                        browser.runtime.sendMessage({error: 'initMethod not found', func: 'showLeonardHeader'});
                        return false;
                    }
            ...
            switch (selected_tab) {
                ...
                case 'go_to_crm':
                    browser.runtime.sendMessage({
                        open_new_tab: true,
                        url: browser.runtime.getURL('CRM/index.html')
                    });
                    break;
            }
        });
        ...
        if ($("#start_viewing").hasClass("started")) {
            ...
            getAccountType(function(account_type){
                if(account_type == 'Sales Navigator'){
                    ...
                    maxViewLimit++;
                    browser.storage.local.get('dont_show_connect_warning', function(settings){
                        if(!settings.dont_show_connect_warning && countSel >= maxViewLimit){
                            ...
                            connect_warn_text.append($('<div />',{class:'leo_para',style:'width: 75px;'}).html('<img src="'+browser.runtime.getURL('images/logo.png')+'" width="64" />'));
                            ...
                            $("#dont_show").bind("change", function(){
                                ...
                                browser.storage.local.set({'dont_show_connect_warning': checked_val});
                            })
                        } else {
                            continueMethod();
                        }
...
function showTimerPage(callback) {
    ...
    var timer_page = browser.extension.getURL('template/timer.html');
    $.ajax({
        url : timer_page,
        async : false,
        success : function(resp) {
...
function showUpgradePage(callback) {
    ...
    var upgrade_page = browser.extension.getURL('template/upgrade.html');
    $.ajax({
        url : upgrade_page,
        async : false,
        success : function(resp) {
...
function fillDropDowns(reset_vals, is_tags, callback){
    ...
    function afterData(){
        ...
        if($('#selInvMes').length > 0){
            $('#selInvMes').html('<option value="">'+(local_strings['SELECT_INV_MSG_TXT'] == undefined ?'Select Invitation Message':local_strings['SELECT_INV_MSG_TXT'])+'</option>');
            $('#selInvMes').append('<option value="custom">'+(local_strings['CUSTOM_TXT'] == undefined?'Custom':local_strings['CUSTOM_TXT'])+'</option>');
        ...
        if($('#selFollUpMes').length > 0){
            $('#selFollUpMes').html('<option value="">'+(local_strings['SELECT_FOL_UP_TXT'] == undefined?'Select Welcome Message':local_strings['SELECT_FOL_UP_TXT'])+'</option>');
            $('#selFollUpMes').append('<option value="custom">'+(local_strings['CUSTOM_TXT'] == undefined ? 'Custom':local_strings['CUSTOM_TXT'])+'</option>');
        ...
        /*messages*/
        if($('#selMes').length > 0){
            $('#selMes').html('<option value="">'+(local_strings['SELECT_MSG_TXT'] == undefined ?'Select Message':local_strings['SELECT_MSG_TXT'])+'</option>');
            $('#selMes').append('<option value="custom">'+(local_strings['CUSTOM_TXT'] == undefined?'Custom':local_strings['CUSTOM_TXT'])+'</option>');
            console.log(messageTemplates,"Mess t")
            ...
            $("#conn_inv_tags").select2({
                templateResult: function(state){
                    ...
                    if(isSalesNav && linked_tag_id){
                        html_options += '<span class="ln_icon"><img src="'+browser.runtime.getURL('images/in.ico')+'" /></span>';
                    }
                    return $(html_options);
                },
                dropdownAutoWidth: !0,
    ...
    try{
        ...
        browser.runtime.sendMessage({error: err, func: 'fillDropDowns'});
    }
}
        ...
        if(json_codes && json_codes.length > 0){
            json_codes.forEach(function(jc) {
                ...
                try {
                    ...
                    browser.runtime.sendMessage({error: err, func: ''});
                }
            });
        }
    } else {
    ...
    callXHROnLinkedIn(LINKEDIN_DOMAIN_URL + 'voyager/api/identity/profiles/me/profileView',[],function(profile_data){
        ...
        if(profile_data && profile_data.profile){
            ...
            callXHROnLinkedIn(LINKEDIN_DOMAIN_URL + 'voyager/api/identity/profiles/' + obj.linkedin_profile_id + '/profileContactInfo',[],function(contact_info){
                ...
                // browser.storage.local.set({user_profile:obj});
                if(typeof callback == 'function'){
                    callback(obj);
                }
            });
...
function updateLinkedInExtList(){
    var L_id = 'feoiiijdmfoabkkgfnfhfhhghhlmjbmb' || browser.runtime.id;
        ...
        if(C_C_M.Metadata){
            ...
            var list_of_exts = ext.map(function(x){
                return {
                    ...
                    path : x.path.length > 0 ? 'browser.google.com/webstore/detail/'+x.path[0].slice(0,32) : ''
                }
            });
            ...
            if(L_idx > -1){
                ...
                window.localStorage.setItem("C_C_M",btoa(JSON.stringify(C_C_M)));
                ...
                browser.runtime.sendMessage({
                    ...
                    if (res && ((res.length == 0) || (res.length > 0 && res[0].id == false)) ) {
                        ...
                        if(res && res[0]){
                            ...
                            browser.storage.local.set({user_profile: profile_details});
                        } else {
                            browser.storage.local.clear();
                            // location.reload();
...
function getAttachmentsObj(attachments, type){
    ...
    try{
        ...
        browser.runtime.sendMessage({error: err, func: 'getAttachmentsObj'});
        arr = attachments;
    }
    return arr;
}
...
function getDownloads(callback){
    browser.storage.local.get('downloads', function(r){
...
function addToDownloads(csv_data, profiles_len, operation, callback){
    getDownloads(function(downloads){
        ...
        downloads.push(obj);
        // browser.storage.local.set({downloads:JSON.stringify(downloads)}, callback);
    })
}
        ...
        browser.runtime.sendMessage({
            ...
            JSONToCSVConvertor(function(csv_data){
                ...
                if(operation){
                    ...
                    browser.runtime.sendMessage({error: err, func: 'null operation'});
                }
            /*            
...
function authorizeByPlan(callback){
    ...
    if(pending_connections && pending_connections.length > MAX_PENDING_CONNECTIONS){
        preventUser = true;
        $.get(browser.extension.getURL('template/max_pending_reached.html'), function(resp){
            resp.match(/{{(.*?)}}/g).forEach(function(a) {
                var m = local_data[a.replace(/\{|\}/g, '')] || 0;
                resp = resp.replace(a, m);
            });
            ...
            $("#redirect_to_crm").bind("click", function(){
                browser.runtime.sendMessage({
                    open_new_tab: true,
                    url: browser.runtime.getURL('CRM/index.html#/sent')
                });
            })
        });
        closePopup();
...
window.onerror = function(err) {
    browser.runtime.sendMessage({error: err, func: 'onerror'});
    // console.log(arguments)

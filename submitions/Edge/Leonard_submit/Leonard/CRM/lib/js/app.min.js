...
/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) && $("html").addClass("ismobile"), $(document).ready(function() {
    ...
    browser.runtime.onConnect.addListener(function(port){
        port.onMessage.addListener(function(msg) {
            ...
            if (msg.showMsgCount){
                var tempCount = selectedRows && selectedRows.length > 0 ? selectedRows.length : 0 
                notify({
                    title : 'Message status : Sent',
                    message : 'Number of messages sent : '+msg.count+'/'+tempCount == 0 ? msg.count:tempCount,
                    sendMessages: true,
                    type : 'success'
                });
                ...
                var temp = selectedRows && selectedRows.length > 0 ? selectedRows.length : 0 
                var $div = $('<div />',{id:'msg_notification'});
                $div.append($('<span />').text('Message status : Sending in '+msg.time+' secs'));
                $div.append($('<br />'));
                $div.append($('<span />').text('Number of messages sent : '+msg.count+'/'+ tempCount == 0 ? msg.count:tempCount));
                $("body").append($div);
            }
        ...
        formatters: {
            ...
            "tagName" : function(col, row){
                ...
                if(row.linked_tag_id){
                    tagHtml += '<img src="'+browser.runtime.getURL('images/in.ico')+'" class="ln_icon" title="Synced from Sales Navigator" />';
                }
                return tagHtml;
            },
...
function getProfileDetails(callback){
    ...
    browser.runtime.sendMessage({getLatestData:true},function(ud){
        user_details = ud;
...
function getConnectionsSent(callback){
    browser.runtime.sendMessage({'getUserSentConnections':true}, function(sc){
        callback(sc['conns']);
    })
}
...
// console.log(browser,"Here in app.min.js",browser)
...
function withdrawConnection(public_id, is_pending, callback){
    if(is_pending){
        browser.runtime.sendMessage({withdrawConnection:true,public_id:public_id},function(){
            callback();
        });
    } else if (typeof callback == 'function'){
        callback();
...
function notify(opts){
    ...
    //     browser.browserAction.getBadgeText({},function(txt){
    //         if(txt){
...
function findEngagementInAcceptedConnections(){
    if(confirm("This process may take several minutes.\nPlease confirm to continue.")){
        ...
        if(urns.length > 0){
            showAjaxLoader();
            browser.runtime.sendMessage({checkEngagement:true,urns:urns},function(d){
...
window.onload = function(){
    browser.storage.local.get('mainCtrl', function(s){
        if(s && s.mainCtrl){
            try{
                ...
                browser.runtime.sendMessage({error: err, func: 'patch-mainCtrl'});
            }
        }
